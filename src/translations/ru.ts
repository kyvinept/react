export default {
  LetsDoThis: "Давайте сделаем это!",
  Mentorship: "Менторство",
  SayHello: "Поздороваться",
  Name: "Имя",
  Email: "Фамилия",
  Message: "Сообщение",
  Submit: "Отправить",
  MyWorks: "Мои работы",
  Emailme: "Напиши мне",
  iOSDeveloper: "iOS Разработчик",
  LanguagesIspeak: "Используемые языки:",
  Technology: "Технологии:",
  DevTools: "Инструменты разработчика:",
  ReactDeveloper: "React Разработчик",
  NodeJSDeveloper: "Node JS Разработчик",
  Loading: "Загрузка...",
  Translating: "Перевод...",
  error404: "404 ошибка",
  Thisisntthedroidorpageyourelookingfor:
    "Это не тот робот (или страница), которого вы ищете.",
  Databases: "Базы данных:",
  TechnologyIuse: "Используемые технологии:",
  IdevelopasimpleBackendpartformybeatifulwebandmobileapps:
    "Я разрабатываю простую Back-end часть для своих красивых веб-приложений и мобильных приложений.",
  DesigntoolsIworkwith: "Инструменты дизайна для работы:",
  ReactNativeDeveloper: "React Native Разработчик",
  Iliketocodethingsfromscratchandenjoybringingideastolifeinthebrowser:
    "Мне нравится создавать вещи с нуля, и мне нравится воплощать идеи в жизнь.",
  IuseReactNativefordevelopingcrossplatformappsIadjustedtocustomersrequeststosavemoneyforcreatingasimpleapp:
    "Я использую React Native для разработки кроссплатформенных приложений. Я приспособился к запросам клиентов, чтобы сэкономить их время на создании простого приложения.",
  IwriteclearnativecodetocreategreatappsforAppleplatform:
    "Я использую нативный код для создания отличных приложений для платформы Apple.",
  Description:
    "Я начал свой путь в роли iOS разработчика. Я работал в большой компании и выполнял проекты, начиная с небольших в маленькой команде на несколько человек, так и больших, в которых было около 30 человек. В дальнейшем выучил Java Script, чтобы дальше совершенствоваться в мобильной разработке и выучил React Native. Я люблю совершенствоваться и постоянно учу что-то новое. Так я выучил React и Node JS, чтобы работать над проектами собственаручно. Фриланс проекты позволили мне набраться комерческого опыта с данными технологии и совершенствоваться в них.",
  HiImVladNicetomeetyou: "Привет, я Влад. Рад встрече.",
  IcodebeautifulthingsandIlovewhatIdo:
    "Я создаю красивые вещи и люблю то, что делаю.",
  FrontendBackendandMobileDeveloper: "Front-end, Back-end и Mobile Разработчик",
  HereareafewdesignprojectsIveworkedonrecentlyWanttoseemore:
    "Вот несколько дизайнерских проектов, над которыми я работал недавно. Хочу увидеть больше? ",
  AppFeatures: "Особенности приложения",
  Illbeintouchwithyoushortly: "Я скоро отвечу!",
  Backtohome: "Вернуться домой",
  MessageReceivedThanks: "Сообщение получено! Спасибо!",
  ThanksForTakingTheTimeToReachOutHowCanIHelpYouToday:
    "Спасибо, что нашли время связаться с нами. Как я могу помочь вам сегодня?",
  LivingLearningLevelingUpOneDayAtATime:
    "Жить, учиться и повышать свой уровень день за днем.",
  HandcraftedByMeVladislav: "Сделано мной вручную © Владислав",
  StartAProject: "Начать проект",
  InterestedInWorkingTogetherWeShouldQueueUpAChatIllBuyTheCoffee:
    "Заинтересованы в совместной работе? Мы должны выделить время и связаться. Я куплю кофе.",
};
